# ========================================
# Final Complete Analysis - Fixed Version
# ========================================
# Comprehensive analysis with all packages loaded
...
if(exists("scns")) {
  ...
  if(length(available_int) >= 2) {
    ...
    venn_intersections <- getFeature(venn_obj, "overlap")
  ...
  if(exists("gastrocs") && "HFDvsSD" %in% names(scns) && "HFDvsSD" %in% names(gastrocs)) {
    tissue_genes <- list(
      SCN = rownames(scns[["HFDvsSD"]]),
      Gastroc = rownames(gastrocs[["HFDvsSD"]])
    )

    pdf("Figures/Cross_Tissue_HFD_Venn.pdf", width = 10, height = 8)
    venn_obj_tissue <- venndetail(tissue_genes)
    plot(venn_obj_tissue)
    dev.off()

    cat("✓ Cross-tissue Venn created: Figures/Cross_Tissue_HFD_Venn.pdf\n")
  }
}
...
# ========================================
# 3. Cross-Tissue Specific Analysis
# ========================================
